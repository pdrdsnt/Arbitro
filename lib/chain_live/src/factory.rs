...
use crate::{
    any_pool::AnyPool, ticks_bitmap::Pair, v2_factory::V2Factory, v2_pool::V2Pool,
    v3_factory::V3Factory, v3_pool::V3Pool, v4_factory::V4Factory, v4_pool::V4Pool,
};

pub trait Factory<P: Provider + Clone> {
    async fn search_pairs(&self, pairs: &Vec<Pair>) -> Vec<AnyPool<P>>;
}
...
impl<P: Provider + Clone> Factory<P> for V2Factory<P> {
    async fn search_pairs(&self, pairs: &Vec<Pair>) -> Vec<AnyPool<P>> {
        let mut pools = vec![];
        for pair in pairs {
            if let Some(v2) = V2Factory::search_pool(&self, pair.a, pair.b).await {
                let v2_contract =
...
impl<P: Provider + Clone> Factory<P> for V3Factory<P> {
    async fn search_pairs(&self, pairs: &Vec<Pair>) -> Vec<AnyPool<P>> {
        ...
        for pair in pairs {
            let pools = V3Factory::search_pools(&self, pair.a, pair.b).await;
...
impl<P: Provider + Clone> Factory<P> for V4Factory<P> {
    async fn search_pairs(&self, pairs: &Vec<Pair>) -> Vec<AnyPool<P>> {
        let mut any_pools: Vec<AnyPool<P>> = Vec::new();
        for pair in pairs {
            let pools: Vec<V4Pool<P>> = V4Factory::search_pools(&self, pair.a, pair.b).await;
