...
pub fn normalize_tick(current_tick: i32, tick_spacing: i32,) -> i32 { current_tick.div_euclid(tick_spacing,) }

/// Calculate the word index in the bitmap for a normalized tick
pub fn word_index(normalized_tick: i32,) -> i32 { normalized_tick.div_euclid(256,) }

/// Generate a range of word indices around the current index with given range
pub fn word_indices(center: i32, range: i32,) -> Vec<i32,> { ((center - range)..=(center + range)).collect() }
...
pub fn collect_ticks_from_map(word_map: &std::collections::HashMap<i32, U256,>, tick_spacing: i32,) -> Vec<i32,> {
    let mut ticks = Vec::new();
...
pub fn tick_from_price(sqrt_price_x96: U256,) -> Option<i32,> {
    ...
    let tick_low =
        ((log2 - U256::from_dec_str("3402992956809132418596140100660247210",).unwrap()) >> 128).as_u32() as i32;
    let tick_high =
        ((log2 + U256::from_dec_str("291339464771989622907027621153398088495",).unwrap()) >> 128).as_u32() as i32;
