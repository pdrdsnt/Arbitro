...
use ethers::types::{Address, Bytes, Transaction, H256};
use tokio::sync::mpsc;
    ...
    ethers::providers::Provider<P,>: Clone,
{
    ...
    pub fn new(
        provider: ethers::providers::Provider<P,>, filter: PoolFilter, poll_interval: Duration,
        tx_sender: mpsc::Sender<MempoolTx,>,
    ) -> Self {
        Self {
            provider,
    ...
    async fn fetch_all(&self,) -> Result<Vec<Transaction,>, ethers::providers::ProviderError,> {
        // no params → empty vec
        self.provider.request("eth_pendingTransactions", Vec::<String,>::new(),).await
    }
}
