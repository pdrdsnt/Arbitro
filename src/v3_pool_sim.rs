...
impl V3PoolSim {
    // Private constructor
    pub fn new(
        address: Address, fee: u32, dex: String, version: String, token0: Token, token1: Token, tick_spacing: i32,
        active_ticks: Vec<Tick,>, liquidity: U256, x96price: U256,
    ) -> Self {
        Self {
            address,
            token0,
    ...
    pub fn trade(&mut self, amount_in: U256, from0: bool,) -> Option<Trade,> {
        // 1. Fee deduction
        let fee_amount = amount_in.checked_mul(U256::from(self.fee,),)?.checked_div(U256::from(1_000_000,),)?;
        ...
        let mut next_tick_index = match self.active_ticks.binary_search_by_key(&current_tick, |t| t.tick,) {
            Ok(i,) => {
                if from0 {
        ...
        while remaining > U256::zero() {
            ...
            let possible = Self::compute_amount_possible(from0, &curr_liq, &curr_price, &next_price,)?;
            ...
            if remaining < possible {
                ...
                // compute out
                let delta = if from0 {
                    curr_liq.checked_mul(new_price.checked_sub(curr_price,)?,)?.checked_div(U256::from(1u128 << 96,),)?
                } else {
                    let inv_curr = (U256::one() << 96).checked_mul(U256::one() << 96,)?.checked_div(curr_price,)?;
                    let inv_new = (U256::one() << 96).checked_mul(U256::one() << 96,)?.checked_div(new_price,)?;
                    curr_liq.checked_mul(inv_curr.checked_sub(inv_new,)?,)?.checked_div(U256::from(1u128 << 96,),)?
                };
    ...
    fn compute_amount_possible(
        from0: bool, available_liquidity: &U256, current_sqrt_price: &U256, next_sqrt_price: &U256,
    ) -> Option<U256,> {
    ...
    fn compute_price_from0(
        amount: &U256, available_liquidity: &U256, current_sqrt_price: &U256, add: bool,
    ) -> Option<U256,> {
    ...
    fn tick_price(target_tick: i32,) -> Option<U256,> {
        ...
        if abs_tick > MAX_TICK as u32 {
            eprintln!("[0] Tick {} exceeds maximum allowed (±{})", target_tick, MAX_TICK);
            return None;
        }
        ...
        let magic_numbers = [
            (0x80000, U256::from_dec_str("366325949420163452428643381347626447728",).unwrap(),),
            (0x40000, U256::from_dec_str("435319348045928502739365042735923241779",).unwrap(),),
            (0x20000, U256::from_dec_str("142576269300693600730609870735819320320",).unwrap(),),
            (0x10000, U256::from_dec_str("366325949420163452428643381347626447728",).unwrap(),),
            (0x8000, U256::from_dec_str("844815322999501822113930908203125000000",).unwrap(),),
            (0x4000, U256::from_dec_str("340265210418746478515625000000000000000",).unwrap(),),
            (0x2000, U256::from_dec_str("215416728668509908758128906250000000000",).unwrap(),),
            (0x1000, U256::from_dec_str("177803588050028359909546862144531250000",).unwrap(),),
            (0x800, U256::from_dec_str("170408874814886611515626254292199532339",).unwrap(),),
            (0x400, U256::from_dec_str("170141183460469231731687303715884105728",).unwrap(),),
            (0x200, U256::from_dec_str("3868562622766813359059763198240802791",).unwrap(),),
            (0x100, U256::from_dec_str("29287344681543793554040907002057611822",).unwrap(),),
            (0x80, U256::from_dec_str("115165952705265534866474743471916972268",).unwrap(),),
            (0x40, U256::from_dec_str("191204177664095573937843702857003287777",).unwrap(),),
            (0x20, U256::from_dec_str("234435455086227615880830483505416481938",).unwrap(),),
            (0x10, U256::from_dec_str("250846047417607353339794883300939388931",).unwrap(),),
            (0x8, U256::from_dec_str("254322734553735582512512255949976165369",).unwrap(),),
            (0x4, U256::from_dec_str("255223438104885656517683320344580614584",).unwrap(),),
            (0x2, U256::from_dec_str("255706422905421325395407485534392863200",).unwrap(),),
        ];
