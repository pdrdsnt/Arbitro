...
impl<V,> MappedVec<V,> {
    ...
    pub fn get_keys(&self,) -> Vec<&H160,> { self.get_keys() }
    /// Returns a reference to the value for the given address, if any.
    pub fn get(&self, addr: &H160,) -> Option<&V,> { self.lookup.get(addr,).map(|&i| &self.entries[i].1,) }

    /// Returns a mutable reference to the value for the given address, if any.
    pub fn get_mut(&mut self, addr: &H160,) -> Option<&mut V,> {
        self.lookup.get(addr,).and_then(|&i| self.entries.get_mut(i,),).map(|(_, v,)| v,)
    }
    ...
    pub fn iter(&self,) -> impl Iterator<Item = &(H160, V,),> { self.entries.iter() }
}
