...
async fn main() -> Result<(), ethers::providers::ProviderError,> {
    ...
    let mut mult_provider: MultiProvider = mult_provider::MultiProvider::new(urls, Duration::from_secs(5,), 3,);
    ...
    let mut chain_src = chain_src::ChainSrc::new(abis_arc.clone(), arc_provider.clone(), _tokens, _dexes,).await;
