...
use crate::{
    err::PoolUpdateError, mult_provider::MultiProvider, tick_math::{self, Tick}, token::Token, v3_pool_sim::V3PoolSim    // ← your V3 sim types
};
...
impl V4PoolSrc {
    ...
    pub async fn update(&mut self) -> Result<(), PoolUpdateError> {
        ...
        let pool_id_call = self
            .factory
            .method::<(H160, H160, u32), H160>("getPool", (self.token0.read().await.address, self.token1.read().await.address, self.fee))
            ?;
        ...
        let state_call = self
            .factory
            .method::<H160, (U256, U256)>("getPoolState", pool_addr)?
            .call()
            .await?;
        ...
        self.active_ticks = idxs
            .into_iter()
            .zip(nets.into_iter())
            .map(|(tick, net)| Tick { tick, liquidityNet: net })
            .collect();
