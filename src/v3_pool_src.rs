...
use crate::{
    err::PoolUpdateError, mult_provider::MultiProvider, tick_math::Tick, token::Token, trade::Trade,
...
impl V3PoolSrc {
    pub async fn new(
        address: Address, token0: Arc<RwLock<Token,>,>, token1: Arc<RwLock<Token,>,>, dex: String, version: String,
        fee: u32, contract: Contract<Provider<MultiProvider,>,>,
    ) -> Self {
    ...
    pub async fn update(&mut self,) -> Result<H160, PoolUpdateError,> {
        let slot0_call_result = self.contract.method::<(), (U256, i32, U256, U256, U256, U256, bool,)>("slot0", (),);
    ...
    async fn fetch_bitmap_words(
        contract: &Contract<Provider<MultiProvider,>,>, word_indices: &[i32],
    ) -> HashMap<i32, U256,> {
    ...
    async fn fetch_bitmap_word(
        contract: &Contract<Provider<MultiProvider,>,>, index: i16,
    ) -> Result<U256, ContractError<Provider<MultiProvider,>,>,> {
        contract.method::<(i16), U256>("tickBitmap", (index),)?.call().await
    }

    /// Find the nearest initialized ticks around the current tick.
    async fn find_nearest_ticks(
        contract: &Contract<Provider<MultiProvider,>,>, current_tick: i32, tick_spacing: i32,
    ) -> Vec<i32,> {
        ...
        let word_indices: Vec<i32,> = (-range..=range).map(|offset| current_word_idx + offset,).collect();
    ...
    async fn fetch_tick_data(contract: &Contract<Provider<MultiProvider,>,>, ticks: &[i32],) -> Vec<Tick,> {
        let tick_futures = ticks
            ...
            .map(|&tick| {
                ...
                async move {
                    ...
                    match contract.method::<_, (u128, i128, U256, U256, i64, U256, u32, bool,)>("ticks", tick,) {
                        Ok(call,) => {
                            // Await the call
        ...
        tick_results.into_iter().filter_map(|result| result,).collect()
    }

    async fn update_active_ticks(&mut self,) {
        let nearest_ticks = V3PoolSrc::find_nearest_ticks(&self.contract, self.current_tick, self.tick_spacing,).await;
