...
impl From<MultiProviderError> for ethers_providers::ProviderError {
    fn from(err: MultiProviderError) -> Self {
        ethers_providers::ProviderError::CustomError(err.to_string())
    }
}
...
impl MultiProvider {
    ...
    pub async fn send_raw(
        ...
        method: &str,
        params: Vec<Value>,
    ) -> Result<Value, MultiProviderError> {
            ...
            if candidates.is_empty() {
                ...
                let earliest = {
                    ...
                    for n in self.nodes.iter() {
                        futures
                            .push(async { n.last_failure.lock().await.map(|t| t + self.backoff) });
                    }
        ...
        Err(MultiProviderError::RequestFailed(
