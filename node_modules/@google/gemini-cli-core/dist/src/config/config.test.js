...
describe('Server Config (config.ts)', () => {
    ...
    describe('checkDeprecatedTools', () => {
        it('should emit a warning when a deprecated tool is in coreTools', async () => {
            ...
            const config = new Config(params);
            await config.initialize();
            expect(mockCoreEvents.emitFeedback).toHaveBeenCalledWith('warning', expect.stringContaining(`The tool '${READ_MANY_FILES_TOOL_NAME}' (or 'ReadManyFilesTool') specified in 'tools.core' is deprecated`));
        });
        it('should emit a warning when a deprecated tool is in allowedTools', async () => {
            ...
            const config = new Config(params);
            await config.initialize();
            expect(mockCoreEvents.emitFeedback).toHaveBeenCalledWith('warning', expect.stringContaining(`The tool '${READ_MANY_FILES_TOOL_NAME}' (or 'ReadManyFilesTool') specified in 'tools.allowed' is deprecated`));
        });
    });
});
