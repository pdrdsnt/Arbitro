...
describe('BaseLlmClient', () => {
    ...
    describe('generateJson - Error Handling', () => {
        it('should throw and report error for empty response after retry exhaustion', async () => {
            ...
            await expect(client.generateJson(defaultOptions)).rejects.toThrow('Failed to generate JSON content: Retry attempts exhausted for invalid content');
            // Verify error reporting details
            expect(reportError).toHaveBeenCalledTimes(1);
            expect(reportError).toHaveBeenCalledWith(expect.any(Error), 'API returned invalid content (empty or unparsable JSON) after all retries.', defaultOptions.contents, 'generateJson-invalid-content');
        });
