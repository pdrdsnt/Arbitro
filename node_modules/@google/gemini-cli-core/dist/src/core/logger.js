    ...
    async _readLogFile() {
        ...
        try {
            ...
            if (error instanceof SyntaxError) {
                debugLogger.debug(`Invalid JSON in log file ${this.logFilePath}. Backing up and starting fresh.`, error);
    ...
    async loadCheckpoint(tag) {
        ...
        try {
            ...
            if (!Array.isArray(parsedContent)) {
                debugLogger.warn(`Checkpoint file at ${path} is not a valid JSON array. Returning empty checkpoint.`);
                return [];
            }
