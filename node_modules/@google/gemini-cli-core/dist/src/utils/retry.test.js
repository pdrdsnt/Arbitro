...
describe('retryWithBackoff', () => {
    ...
    it('should not retry if shouldRetry returns false', async () => {
        ...
        const shouldRetryOnError = (error) => !(error instanceof NonRetryableError);
        const promise = retryWithBackoff(mockFn, {
            shouldRetryOnError,
            initialDelayMs: 10,
        });
    ...
    it('should throw an error if maxAttempts is not a positive number', async () => {
        ...
        // Test with 0
        await expect(retryWithBackoff(mockFn, { maxAttempts: 0 })).rejects.toThrow('maxAttempts must be a positive number.');
    ...
    describe('Fetch error retries', () => {
        ...
        it.each([false, undefined])('should not retry on specific fetch error when retryFetchErrors is %s', async (retryFetchErrors) => {
