...
describe('parsePrompt', () => {
    ...
    it('should detect unsupported protocols (httpshttps://)', () => {
        const prompt = 'Summarize httpshttps://github.com/JuliaLang/julia/issues/58346';
        const { validUrls, errors } = parsePrompt(prompt);
        expect(validUrls).toHaveLength(0);
        expect(errors).toHaveLength(1);
        expect(errors[0]).toContain('Unsupported protocol');
        expect(errors[0]).toContain('httpshttps://github.com/JuliaLang/julia/issues/58346');
    });
...
describe('WebFetchTool', () => {
    ...
    describe('validateToolParamValues', () => {
        it('should throw if prompt is empty', () => {
            ...
            expect(() => tool.build({ prompt: '' })).toThrow("The 'prompt' parameter cannot be empty");
        });
        it('should throw if prompt contains no URLs', () => {
            const tool = new WebFetchTool(mockConfig);
            expect(() => tool.build({ prompt: 'hello world' })).toThrow("The 'prompt' must contain at least one valid URL");
        });
    ...
    describe('execute', () => {
        ...
        it('should log telemetry when falling back due to private IP', async () => {
            ...
            expect(logWebFetchFallbackAttempt).toHaveBeenCalledWith(mockConfig, expect.any(WebFetchFallbackAttemptEvent));
    ...
    describe('Message Bus Integration', () => {
        ...
        it('should reject promise when confirmation is denied via message bus', async () => {
            ...
            await expect(confirmationPromise).rejects.toThrow('Tool execution for "WebFetch" denied by policy.');
        });
