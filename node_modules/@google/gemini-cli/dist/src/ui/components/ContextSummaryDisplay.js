...
export const ContextSummaryDisplay = ({ geminiMdFileCount, contextFileNames, mcpServers, blockedMcpServers, ideContext, }) => {
    ...
    if (isNarrow) {
        return (_jsxs(Box, { flexDirection: "column", paddingX: 1, children: [_jsx(Text, { color: theme.text.secondary, children: "Using:" }), summaryParts.map((part, index) => (_jsxs(Text, { color: theme.text.secondary, children: ['  ', "- ", part] }, index)))] }));
    }
    return (_jsx(Box, { paddingX: 1, children: _jsxs(Text, { color: theme.text.secondary, children: ["Using: ", summaryParts.join(' | ')] }) }));
};
