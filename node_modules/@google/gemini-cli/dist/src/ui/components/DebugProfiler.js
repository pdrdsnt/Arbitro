...
export const profiler = {
    ...
    registerFlickerHandler(constrainHeight) {
        const flickerHandler = () => {
            ...
            if (!this.hasLoggedFirstFlicker) {
                ...
                appEvents.emit(AppEvent.LogError, 'A flicker frame was detected. This will cause UI instability. Type `/profile` for more info.');
            }
        };
...
export const DebugProfiler = () => {
    ...
    return (_jsxs(Text, { color: theme.status.warning, children: ["Renders: ", profiler.numFrames, " (total),", ' ', _jsxs(Text, { color: theme.status.error, children: [profiler.totalIdleFrames, " (idle)"] }), ",", ' ', _jsxs(Text, { color: theme.status.error, children: [profiler.totalFlickerFrames, " (flicker)"] })] }, forceRefresh));
};
