...
export function PermissionsModifyTrustDialog({ onExit, addItem, }) {
    ...
    const { cwd, currentTrustLevel, isInheritedTrustFromParent, isInheritedTrustFromIde, needsRestart, updateTrustLevel, commitTrustLevelChange, } = usePermissionsModifyTrust(onExit, addItem);
    useKeypress((key) => {
        if (key.name === 'escape') {
            onExit();
        }
