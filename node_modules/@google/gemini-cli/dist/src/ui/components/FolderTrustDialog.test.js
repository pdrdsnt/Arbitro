...
describe('FolderTrustDialog', () => {
    ...
    it('should display exit message and call process.exit and not call onSelect when escape is pressed', async () => {
        ...
        const { lastFrame, stdin } = renderWithProviders(_jsx(FolderTrustDialog, { onSelect: onSelect, isRestarting: false }));
        act(() => {
            stdin.write('\u001b[27u'); // Press kitty escape key
        });
        await waitFor(() => {
            expect(lastFrame()).toContain('A folder trust level must be selected to continue. Exiting since escape was pressed.');
        });
        await waitFor(() => {
    ...
    it('should call relaunchApp when isRestarting is true', async () => {
        ...
        renderWithProviders(_jsx(FolderTrustDialog, { onSelect: vi.fn(), isRestarting: true }));
