...
describe('<ExtensionsList />', () => {
    ...
    const mockUIState = (extensionsUpdateState) => {
    ...
    it('should render "No extensions installed." if there are no extensions', () => {
        mockUIState(new Map());
        const { lastFrame, unmount } = render(_jsx(ExtensionsList, { extensions: [] }));
        expect(lastFrame()).toContain('No extensions installed.');
        unmount();
    });
    it('should render a list of extensions with their version and status', () => {
        mockUIState(new Map());
        const { lastFrame, unmount } = render(_jsx(ExtensionsList, { extensions: mockExtensions }));
    ...
    it('should display "unknown state" if an extension has no update state', () => {
        mockUIState(new Map());
        const { lastFrame, unmount } = render(_jsx(ExtensionsList, { extensions: [mockExtensions[0]] }));
...
//# sourceMappingURL=ExtensionsList.test.js.map