...
import { DEFAULT_GEMINI_FLASH_LITE_MODEL, DEFAULT_GEMINI_FLASH_MODEL, DEFAULT_GEMINI_MODEL, DEFAULT_GEMINI_MODEL_AUTO, } from '@google/gemini-cli-core';
...
describe('<ModelDialog />', () => {
    ...
    it('renders the title and help text', () => {
        ...
        expect(lastFrame()).toContain('> To use a specific Gemini model on startup, use the --model flag.');
        unmount();
    });
    ...
    it('updates initialIndex when config context changes', () => {
        ...
        const { rerender, unmount } = render(_jsx(ConfigContext.Provider, { value: { getModel: mockGetModel }, children: _jsx(ModelDialog, { onClose: vi.fn() }) }));
        expect(mockedSelect.mock.calls[0][0].initialIndex).toBe(0);
        mockGetModel.mockReturnValue(DEFAULT_GEMINI_FLASH_LITE_MODEL);
        const newMockConfig = { getModel: mockGetModel };
