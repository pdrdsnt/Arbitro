...
describe('ThemeDialog Snapshots', () => {
    ...
    it('should render correctly in theme selection mode', () => {
        ...
        const { lastFrame } = render(_jsx(SettingsContext.Provider, { value: settings, children: _jsx(KeypressProvider, { children: _jsx(ThemeDialog, { ...baseProps, settings: settings }) }) }));
        expect(lastFrame()).toMatchSnapshot();
    });
    it('should render correctly in scope selector mode', async () => {
        const settings = createMockSettings();
        const { lastFrame, stdin } = render(_jsx(SettingsContext.Provider, { value: settings, children: _jsx(KeypressProvider, { children: _jsx(ThemeDialog, { ...baseProps, settings: settings }) }) }));
    ...
    it('should call onCancel when ESC is pressed', async () => {
        ...
        const settings = createMockSettings();
        const { stdin } = render(_jsx(SettingsContext.Provider, { value: settings, children: _jsx(KeypressProvider, { children: _jsx(ThemeDialog, { ...baseProps, onCancel: mockOnCancel, settings: settings }) }) }));
        act(() => {
            stdin.write('\x1b');
        });
        await waitFor(() => {
