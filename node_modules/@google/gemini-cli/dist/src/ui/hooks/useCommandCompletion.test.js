...
const setupMocks = ({ atSuggestions = [], slashSuggestions = [], isLoading = false, isPerfectMatch = false, slashCompletionRange = { completionStart: 0, completionEnd: 0 }, }) => {
...
describe('useCommandCompletion', () => {
    ...
    const renderCommandCompletionHook = (initialText, cursorOffset, shellModeActive = false) => {
        let hookResult;
        function TestComponent() {
            const textBuffer = useTextBufferForTest(initialText, cursorOffset);
            const completion = useCommandCompletion(textBuffer, testDirs, testRootDir, [], mockCommandContext, false, shellModeActive, mockConfig);
    ...
    describe('prompt completion filtering', () => {
        it('should not trigger prompt completion for line comments', async () => {
            ...
            function TestComponent() {
                ...
                const completion = useCommandCompletion(textBuffer, testDirs, testRootDir, [], mockCommandContext, false, false, mockConfig);
                hookResult = { ...completion, textBuffer };
                return null;
            }
            render(_jsx(TestComponent, {}));
