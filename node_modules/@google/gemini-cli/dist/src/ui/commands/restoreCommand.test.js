...
describe('restoreCommand', () => {
    ...
    it('should return the command if checkpointing is enabled', () => {
        expect(restoreCommand(mockConfig)).toEqual(expect.objectContaining({
            name: 'restore',
    ...
    describe('action', () => {
        it('should return an error if temp dir is not found', async () => {
            vi.mocked(mockConfig.storage.getProjectTempCheckpointsDir).mockReturnValue('');
        ...
        it('should restore a tool call and project state', async () => {
            ...
            await fs.writeFile(path.join(checkpointsDir, 'my-checkpoint.json'), JSON.stringify(toolCallData));
            const command = restoreCommand(mockConfig);
            ...
            expect(mockGitService.restoreProjectFromSnapshot).toHaveBeenCalledWith(toolCallData.commitHash);
    ...
    it('should return an error for a checkpoint file missing the toolCall property', async () => {
        ...
        await fs.writeFile(path.join(checkpointsDir, `${checkpointName}.json`), JSON.stringify({ history: [] }));
        const command = restoreCommand(mockConfig);
        expect(await command?.action?.(mockContext, checkpointName)).toEqual({
            type: 'message',
            messageType: 'error',
...
//# sourceMappingURL=restoreCommand.test.js.map