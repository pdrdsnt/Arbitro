...
import { AuthType, OutputFormat, makeFakeConfig, } from '@google/gemini-cli-core';
...
describe('validateNonInterActiveAuth', () => {
    let originalEnvGeminiApiKey;
    let originalEnvVertexAi;
    let originalEnvGcp;
    let consoleErrorSpy;
    let processExitSpy;
    let refreshAuthMock;
    ...
    it('exits if no auth type is configured or env vars set', async () => {
        ...
        try {
            await validateNonInteractiveAuth(undefined, undefined, nonInteractiveConfig, mockSettings);
            expect.fail('Should have exited');
        }
        catch (e) {
            expect(e.message).toContain('process.exit(1) called');
        }
        expect(consoleErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Please set an Auth method'));
    ...
    it('uses configuredAuthType over environment variables', async () => {
        ...
        await validateNonInteractiveAuth(AuthType.LOGIN_WITH_GOOGLE, undefined, nonInteractiveConfig, mockSettings);
        expect(refreshAuthMock).toHaveBeenCalledWith(AuthType.LOGIN_WITH_GOOGLE);
    });
    it('exits if validateAuthMethod returns error', async () => {
        ...
        try {
            await validateNonInteractiveAuth(AuthType.USE_GEMINI, undefined, nonInteractiveConfig, mockSettings);
            expect.fail('Should have exited');
        }
        catch (e) {
            expect(e.message).toContain('process.exit(1) called');
    ...
    it('skips validation if useExternalAuth is true', async () => {
        ...
        await validateNonInteractiveAuth('invalid-auth-type', true, // useExternalAuth = true
        nonInteractiveConfig, mockSettings);
    ...
    it('exits if configuredAuthType does not match enforcedAuthType', async () => {
        ...
        expect(consoleErrorSpy).toHaveBeenCalledWith("The enforced authentication type is 'oauth-personal', but the current type is 'gemini-api-key'. Please re-authenticate with the correct type.");
        expect(processExitSpy).toHaveBeenCalledWith(1);
    });
    ...
    describe('JSON output mode', () => {
        it('prints JSON error when no auth is configured and exits with code 1', async () => {
            ...
            expect(payload.error.message).toContain('Please set an Auth method in your');
        });
        it('prints JSON error when enforced auth mismatches current auth and exits with code 1', async () => {
            ...
            expect(thrown?.message).toBe('process.exit(1) called');
            {
                ...
                expect(payload.error.type).toBe('Error');
                expect(payload.error.code).toBe(1);
                expect(payload.error.message).toContain("The enforced authentication type is 'gemini-api-key', but the current type is 'oauth-personal'. Please re-authenticate with the correct type.");
            }
        });
...
//# sourceMappingURL=validateNonInterActiveAuth.test.js.map