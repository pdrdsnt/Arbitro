...
describe('extensions validate command', () => {
    it('should fail if no path is provided', () => {
        ...
        expect(() => validationParser.parse('validate')).toThrow('Not enough non-option arguments: got 0, need at least 1');
    });
});
describe('handleValidate', () => {
    ...
    it('should validate an extension from a local dir', async () => {
        ...
        expect(debugLoggerLogSpy).toHaveBeenCalledWith('Extension local-ext-name has been successfully validated.');
    });
    it('should throw an error if the extension name is invalid', async () => {
        ...
        expect(debugLoggerErrorSpy).toHaveBeenCalledWith(expect.stringContaining('Invalid extension name: "INVALID_NAME". Only letters (a-z, A-Z), numbers (0-9), and dashes (-) are allowed.'));
    ...
    it('should warn if version is not formatted with semver', async () => {
        ...
        expect(debugLoggerWarnSpy).toHaveBeenCalledWith(expect.stringContaining("Version '1' does not appear to be standard semver (e.g., 1.0.0)."));
        expect(debugLoggerLogSpy).toHaveBeenCalledWith('Extension valid-name has been successfully validated.');
    });
    it('should throw an error if context files are missing', async () => {
        ...
        expect(debugLoggerErrorSpy).toHaveBeenCalledWith(expect.stringContaining('The following context files referenced in gemini-extension.json are missing: contextFile.md'));
        expect(processSpy).toHaveBeenCalledWith(1);
    });
});
