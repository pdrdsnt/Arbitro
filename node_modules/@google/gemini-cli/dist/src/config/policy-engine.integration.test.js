...
describe('Policy Engine Integration Tests', () => {
    describe('Policy configuration produces valid PolicyEngine config', () => {
        it('should create a working PolicyEngine from basic settings', async () => {
            ...
            expect(engine.check({ name: 'run_shell_command' }, undefined)).toBe(PolicyDecision.ALLOW);
            // Excluded tool should be denied
            expect(engine.check({ name: 'write_file' }, undefined)).toBe(PolicyDecision.DENY);
            // Other write tools should ask user
            expect(engine.check({ name: 'replace' }, undefined)).toBe(PolicyDecision.ASK_USER);
            // Unknown tools should use default
            expect(engine.check({ name: 'unknown_tool' }, undefined)).toBe(PolicyDecision.ASK_USER);
        });
        it('should handle MCP server wildcard patterns correctly', async () => {
            ...
            expect(engine.check({ name: 'allowed-server__tool1' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'allowed-server__another_tool' }, undefined)).toBe(PolicyDecision.ALLOW);
            // Tools from trusted server should be allowed
            expect(engine.check({ name: 'trusted-server__tool1' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'trusted-server__special_tool' }, undefined)).toBe(PolicyDecision.ALLOW);
            ...
            expect(engine.check({ name: 'blocked-server__any_tool' }, undefined)).toBe(PolicyDecision.DENY);
            // Tools from unknown servers should use default
            expect(engine.check({ name: 'unknown-server__tool' }, undefined)).toBe(PolicyDecision.ASK_USER);
        });
        ...
        it('should handle complex mixed configurations', async () => {
            ...
            expect(engine.check({ name: 'read_file' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'list_directory' }, undefined)).toBe(PolicyDecision.ALLOW);
            // But glob is explicitly excluded, so it should be denied
            expect(engine.check({ name: 'glob' }, undefined)).toBe(PolicyDecision.DENY);
            // Replace should ask user (normal write tool behavior)
            expect(engine.check({ name: 'replace' }, undefined)).toBe(PolicyDecision.ASK_USER);
            // Explicitly allowed tools
            expect(engine.check({ name: 'custom-tool' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'my-server__special-tool' }, undefined)).toBe(PolicyDecision.ALLOW);
            // MCP server tools
            expect(engine.check({ name: 'allowed-server__tool' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'trusted-server__tool' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'blocked-server__tool' }, undefined)).toBe(PolicyDecision.DENY);
            // Write tools should ask by default
            expect(engine.check({ name: 'write_file' }, undefined)).toBe(PolicyDecision.ASK_USER);
        });
        it('should handle YOLO mode correctly', async () => {
            ...
            expect(engine.check({ name: 'run_shell_command' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'write_file' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'unknown_tool' }, undefined)).toBe(PolicyDecision.ALLOW);
            // But explicitly excluded tools should still be denied
            expect(engine.check({ name: 'dangerous-tool' }, undefined)).toBe(PolicyDecision.DENY);
        });
        it('should handle AUTO_EDIT mode correctly', async () => {
            ...
            expect(engine.check({ name: 'replace' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'write_file' }, undefined)).toBe(PolicyDecision.ALLOW);
            // Other tools should follow normal rules
            expect(engine.check({ name: 'run_shell_command' }, undefined)).toBe(PolicyDecision.ASK_USER);
        });
        it('should verify priority ordering works correctly in practice', async () => {
            ...
            expect(engine.check({ name: 'blocked-tool' }, undefined)).toBe(PolicyDecision.DENY);
            expect(engine.check({ name: 'blocked-server__any' }, undefined)).toBe(PolicyDecision.DENY);
            expect(engine.check({ name: 'specific-tool' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'trusted-server__any' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'mcp-server__any' }, undefined)).toBe(PolicyDecision.ALLOW);
            expect(engine.check({ name: 'glob' }, undefined)).toBe(PolicyDecision.ALLOW);
        });
        it('should handle edge case: MCP server with both trust and exclusion', async () => {
            ...
            expect(engine.check({ name: 'conflicted-server__tool' }, undefined)).toBe(PolicyDecision.DENY);
        });
        it('should handle edge case: specific tool allowed but server excluded', async () => {
            ...
            expect(engine.check({ name: 'my-server__special-tool' }, undefined)).toBe(PolicyDecision.DENY);
            expect(engine.check({ name: 'my-server__other-tool' }, undefined)).toBe(PolicyDecision.DENY);
        });
        it('should verify non-interactive mode transformation', async () => {
            ...
            expect(engine.check({ name: 'unknown_tool' }, undefined)).toBe(PolicyDecision.DENY);
            expect(engine.check({ name: 'run_shell_command' }, undefined)).toBe(PolicyDecision.DENY);
        });
        it('should handle empty settings gracefully', async () => {
            ...
            expect(engine.check({ name: 'replace' }, undefined)).toBe(PolicyDecision.ASK_USER);
            // Unknown tools should use default
            expect(engine.check({ name: 'unknown' }, undefined)).toBe(PolicyDecision.ASK_USER);
        });
