...
export async function updateExtension(extension, extensionManager, currentState, dispatchExtensionStateUpdate, enableExtensionReloading) {
    ...
    try {
        const previousExtensionConfig = extensionManager.loadExtensionConfig(extension.path);
        let updatedExtension;
        try {
            updatedExtension = await extensionManager.installOrUpdateExtension(installMetadata, previousExtensionConfig);
        }
        catch (e) {
            dispatchExtensionStateUpdate({
                type: 'SET_STATE',
...
export async function updateAllUpdatableExtensions(extensions, extensionsState, extensionManager, dispatch, enableExtensionReloading) {
    return (await Promise.all(extensions
        .filter((extension) => extensionsState.get(extension.name)?.status ===
        ExtensionUpdateState.UPDATE_AVAILABLE)
        .map((extension) => updateExtension(extension, extensionManager, extensionsState.get(extension.name).status, dispatch, enableExtensionReloading)))).filter((updateInfo) => !!updateInfo);
}
export async function checkForAllExtensionUpdates(extensions, extensionManager, dispatch) {
    ...
    try {
        ...
        for (const extension of extensions) {
            ...
            promises.push(checkForExtensionUpdate(extension, extensionManager).then((state) => dispatch({
                type: 'SET_STATE',
