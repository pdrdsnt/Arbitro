...
function getExpectedEscapedArgForPlatform(arg) {
...
function createPromptPipelineContent(text) {
...
describe('ShellProcessor', () => {
    ...
    it('should throw an error if config is missing', async () => {
        ...
        const prompt = createPromptPipelineContent('!{ls}');
        const contextWithoutConfig = createMockCommandContext({
            services: {
                config: null,
            },
        });
        await expect(processor.process(prompt, contextWithoutConfig)).rejects.toThrow(/Security configuration not loaded/);
    });
    it('should not change the prompt if no shell injections are present', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('This is a simple prompt with no injections.');
    ...
    it('should process a single valid shell injection if allowed', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('The current status is: !{git status}');
        mockCheckCommandPermissions.mockReturnValue({
            allAllowed: true,
            disallowedCommands: [],
        });
        ...
        const result = await processor.process(prompt, context);
        expect(mockCheckCommandPermissions).toHaveBeenCalledWith('git status', expect.any(Object), context.session.sessionShellAllowlist);
        expect(mockShellExecute).toHaveBeenCalledWith('git status', expect.any(String), expect.any(Function), expect.any(Object), false, expect.any(Object));
    ...
    it('should process multiple valid shell injections if all are allowed', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('!{git status} in !{pwd}');
        mockCheckCommandPermissions.mockReturnValue({
            allAllowed: true,
            disallowedCommands: [],
        });
    ...
    it('should throw ConfirmationRequiredError if a command is not allowed in default mode', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('Do something dangerous: !{rm -rf /}');
        mockCheckCommandPermissions.mockReturnValue({
            allAllowed: false,
            disallowedCommands: ['rm -rf /'],
        });
        await expect(processor.process(prompt, context)).rejects.toThrow(ConfirmationRequiredError);
    });
    it('should NOT throw ConfirmationRequiredError if a command is not allowed but approval mode is YOLO', async () => {
        ...
        expect(mockShellExecute).toHaveBeenCalledWith('rm -rf /', expect.any(String), expect.any(Function), expect.any(Object), false, expect.any(Object));
    ...
    it('should still throw an error for a hard-denied command even in YOLO mode', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('Do something forbidden: !{reboot}');
        mockCheckCommandPermissions.mockReturnValue({
            allAllowed: false,
        ...
        mockConfig.getApprovalMode.mockReturnValue(ApprovalMode.YOLO);
        await expect(processor.process(prompt, context)).rejects.toThrow(/Blocked command: "reboot". Reason: System commands are blocked/);
    ...
    it('should throw ConfirmationRequiredError with multiple commands if multiple are disallowed', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('!{cmd1} and !{cmd2}');
    ...
    it('should not execute any commands if at least one requires confirmation', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('First: !{echo "hello"}, Second: !{rm -rf /}');
        mockCheckCommandPermissions.mockImplementation((cmd) => {
            if (cmd.includes('rm')) {
    ...
    it('should only request confirmation for disallowed commands in a mixed prompt', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('Allowed: !{ls -l}, Disallowed: !{rm -rf /}');
        mockCheckCommandPermissions.mockImplementation((cmd) => ({
    ...
    it('should execute all commands if they are on the session allowlist', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('Run !{cmd1} and !{cmd2}');
        ...
        const result = await processor.process(prompt, context);
        expect(mockCheckCommandPermissions).toHaveBeenCalledWith('cmd1', expect.any(Object), context.session.sessionShellAllowlist);
        expect(mockCheckCommandPermissions).toHaveBeenCalledWith('cmd2', expect.any(Object), context.session.sessionShellAllowlist);
        expect(mockShellExecute).toHaveBeenCalledTimes(2);
    ...
    it('should trim whitespace from the command inside the injection before interpolation', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('Files: !{  ls {{args}} -l  }');
        ...
        await processor.process(prompt, context);
        expect(mockCheckCommandPermissions).toHaveBeenCalledWith(expectedCommand, expect.any(Object), context.session.sessionShellAllowlist);
        expect(mockShellExecute).toHaveBeenCalledWith(expectedCommand, expect.any(String), expect.any(Function), expect.any(Object), false, expect.any(Object));
    });
    it('should handle an empty command inside the injection gracefully (skips execution)', async () => {
        const processor = new ShellProcessor('test-command');
        const prompt = createPromptPipelineContent('This is weird: !{}');
        const result = await processor.process(prompt, context);
    ...
    describe('Error Reporting', () => {
        it('should append exit code and command name on failure', async () => {
            const processor = new ShellProcessor('test-command');
            const prompt = createPromptPipelineContent('!{cmd}');
            mockShellExecute.mockReturnValue({
                result: Promise.resolve({
                    ...SUCCESS_RESULT,
        ...
        it('should throw a detailed error if the shell fails to spawn', async () => {
            const processor = new ShellProcessor('test-command');
            const prompt = createPromptPipelineContent('!{bad-command}');
            ...
            await expect(processor.process(prompt, context)).rejects.toThrow("Failed to start shell command in 'test-command': spawn EACCES. Command: bad-command");
        });
        it('should report abort status with command name if aborted', async () => {
            const processor = new ShellProcessor('test-command');
            const prompt = createPromptPipelineContent('!{long-running-command}');
    ...
    describe('Context-Aware Argument Interpolation ({{args}})', () => {
        ...
        it('should perform raw replacement if no shell injections are present (optimization path)', async () => {
            const processor = new ShellProcessor('test-command');
            const prompt = createPromptPipelineContent('The user said: {{args}}');
            const result = await processor.process(prompt, context);
        ...
        it('should perform raw replacement outside !{} blocks', async () => {
            const processor = new ShellProcessor('test-command');
            const prompt = createPromptPipelineContent('Outside: {{args}}. Inside: !{echo "hello"}');
            mockShellExecute.mockReturnValue({
        ...
        it('should perform escaped replacement inside !{} blocks', async () => {
            const processor = new ShellProcessor('test-command');
            const prompt = createPromptPipelineContent('Command: !{grep {{args}} file.txt}');
            mockShellExecute.mockReturnValue({
        ...
        it('should handle both raw (outside) and escaped (inside) injection simultaneously', async () => {
            const processor = new ShellProcessor('test-command');
            const prompt = createPromptPipelineContent('User "({{args}})" requested search: !{search {{args}}}');
            mockShellExecute.mockReturnValue({
        ...
        it('should perform security checks on the final, resolved (escaped) command', async () => {
            const processor = new ShellProcessor('test-command');
            const prompt = createPromptPipelineContent('!{rm {{args}}}');
            const expectedEscapedArgs = getExpectedEscapedArgForPlatform(rawArgs);
            ...
            await expect(processor.process(prompt, context)).rejects.toThrow(ConfirmationRequiredError);
            expect(mockCheckCommandPermissions).toHaveBeenCalledWith(expectedResolvedCommand, expect.any(Object), context.session.sessionShellAllowlist);
        });
        it('should report the resolved command if a hard denial occurs', async () => {
            ...
            await expect(processor.process(prompt, context)).rejects.toThrow(`Blocked command: "${expectedResolvedCommand}". Reason: It is forbidden.`);
        });
    });
    describe('Real-World Escaping Scenarios', () => {
        it('should correctly handle multiline arguments', async () => {
            ...
            const prompt = createPromptPipelineContent('Commit message: !{git commit -m {{args}}}');
        ...
        it.each([
            ...
            const prompt = createPromptPipelineContent('!{echo {{args}}}');
...
//# sourceMappingURL=shellProcessor.test.js.map