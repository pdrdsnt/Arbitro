    ...
    try {
        ...
        return new Promise((resolve, reject) => {
            ...
            sandboxProcess?.on('close', (code, signal) => {
                process.stdin.resume();
                if (code !== 0 && code !== null) {
                    debugLogger.log(`Sandbox process exited with code: ${code}, signal: ${signal}`);
                }
                resolve(code ?? 1);
            });
        });
...
async function imageExists(sandbox, image) {
    return new Promise((resolve) => {
        ...
        checkProcess.on('error', (err) => {
            debugLogger.warn(`Failed to start '${sandbox}' command for image check: ${err.message}`);
            resolve(false);
        });
...
async function pullImage(sandbox, image) {
    ...
    return new Promise((resolve) => {
        ...
        const onError = (err) => {
            debugLogger.warn(`Failed to start '${sandbox} pull ${image}' command: ${err.message}`);
            cleanup();
            resolve(false);
        };
        const onClose = (code) => {
            if (code === 0) {
                ...
                debugLogger.warn(`Failed to pull image ${image}. '${sandbox} pull ${image}' exited with code ${code}.`);
                if (stderrData.trim()) {
...
async function ensureSandboxImageIsPresent(sandbox, image) {
    ...
    if (await pullImage(sandbox, image)) {
        ...
        if (await imageExists(sandbox, image)) {
            ...
            debugLogger.warn(`Sandbox image ${image} still not found after a pull attempt. This might indicate an issue with the image name or registry, or the pull command reported success but failed to make the image available.`);
            return false;
        }
    }
    coreEvents.emitFeedback('error', `Failed to obtain sandbox image ${image} after check and pull attempt.`);
