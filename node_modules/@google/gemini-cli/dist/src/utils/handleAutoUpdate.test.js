...
describe('handleAutoUpdate', () => {
    ...
    it.each([PackageManager.NPX, PackageManager.PNPX, PackageManager.BUNX])('should suppress update notifications when running via %s', (packageManager) => {
        mockGetInstallationInfo.mockReturnValue({
    ...
    it('should emit "update-failed" when the update process fails', async () => {
        ...
        expect(mockUpdateEventEmitter.emit).toHaveBeenCalledWith('update-failed', {
            message: 'Automatic update failed. Please try updating manually. (command: npm i -g @google/gemini-cli@2.0.0, stderr: An error occurred)',
        });
    });
...
//# sourceMappingURL=handleAutoUpdate.test.js.map