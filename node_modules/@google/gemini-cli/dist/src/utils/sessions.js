...
export async function listSessions(config) {
    ...
    sessions
        ...
        console.log(`  ${index + 1}. ${session.firstUserMessage} (${time}${current}) [${session.id}]`);
    });
}
export async function deleteSession(config, sessionIndex) {
    const sessionSelector = new SessionSelector(config);
    const sessions = await sessionSelector.listSessions();
    if (sessions.length === 0) {
    ...
    if (sessionByUuid) {
        ...
        if (isNaN(index) || index < 1 || index > sessions.length) {
            console.error(`Invalid session identifier "${sessionIndex}". Use --list-sessions to see available sessions.`);
            return;
        }
    ...
    try {
        ...
        console.log(`Deleted session ${sessionToDelete.index}: ${sessionToDelete.firstUserMessage} (${time})`);
    }
    catch (error) {
        console.error(`Failed to delete session: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
