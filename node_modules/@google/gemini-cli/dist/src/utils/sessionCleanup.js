...
export async function cleanupExpiredSessions(config, settings) {
    ...
    try {
            ...
            try {
                ...
                if (config.getDebugMode()) {
                    if (sessionToDelete.sessionInfo === null) {
                        ...
                        debugLogger.debug(`Deleted expired session: ${sessionToDelete.sessionInfo.id} (${sessionToDelete.sessionInfo.lastUpdated})`);
                    }
                }
                result.deleted++;
            }
        ...
        if (config.getDebugMode() && result.deleted > 0) {
            debugLogger.debug(`Session cleanup: deleted ${result.deleted}, skipped ${result.skipped}, failed ${result.failed}`);
        }
    }
    catch (error) {
